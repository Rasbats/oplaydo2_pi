...
void pi_OverlayFactory::DrawOLBitmap( const wxBitmap &bitmap, wxCoord x, wxCoord y, bool usemask )
{
    wxBitmap bmp;
    if( x < 0 || y < 0 ) {
        int dx = ( x < 0 ? -x : 0 );
...
void pi_OverlayFactory::DrawGLLabels(pi_OverlayFactory *pof, wxDC *dc,
                                PlugIn_ViewPort *vp,
...
void pi_OverlayFactory::drawGLPolygons(pi_OverlayFactory *pof, wxDC *dc,
                                PlugIn_ViewPort *vp,
                                wxImage &imageLabel, double myLat, double myLon, int offset )
{
